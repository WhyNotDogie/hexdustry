package dev.dogie.hexdustry.registry
...
typealias RegistrarEntry<T> = HexdustryRegistrar<T>.Entry<out T>

abstract class HexdustryRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hexdustry.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexdustryRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
